...
const NotiPicker: FC<NotiPickerProps> = ({ visible, setVisible, onCancel, onConfirm }) => {
      const feedIcon = [
        {
          label: 'class',
    ...
    style={[{
        ...
        style={[
            ...
            currentStyle?.background_style]}>
                <View><Text style={[{textAlign: 'center', fontSize: 18, fontWeight: 'bold'},
                    currentStyle?.schedule_text_style
                ]}>请选择要推送的消息</Text></View>
                <View style={{paddingTop: 10}}><Text style={[{textAlign: 'center', fontSize: 14}
                    , currentStyle?.notification_text_style
                ]}>选择以后将为您推送以下消息</Text></View>
            </View>
            <View
            style={[{
                position: 'absolute',
            ...
            currentStyle?.background_style]}>
                ...
                size={24}
                onPress={() => {setVisible(false)}}></MaterialIcons>
            </View>
        {
            feedIcon.map(item => (
            <View
